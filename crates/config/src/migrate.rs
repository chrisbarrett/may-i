...
/// Flatten nested associative combinators and eliminate double-not.
///
/// - `(and (and e1 ... eN) ...)` → `(and e1 ... eN ...)`
/// - `(or (or e1 ... eN) ...)` → `(or e1 ... eN ...)`
/// - `(not (not e))` → `e`
fn flatten_combinators(node: &CstNode) -> Option<Box<CstNode>> {
    let children = node.as_list()?;
    if children.len() < 2 {
        return None;
    }
...
///
/// `(and ... (effect ...) ...)` where the effect has complexity ≤ 3
/// → `(when (and ...) (effect ...))` (or `(when PRED (effect ...))` if only
/// one predicate remains).
///
/// The effect may appear at any position among the `and` children — not just
/// last — because `rule_collapse_effects` preserves source order and the
/// original `(effect ...)` may precede `(args ...)`.
///
...
mod tests {
    ...
    fn test_and_leading_effect_to_when() {
        // (and (effect :allow) pred) → (when pred (effect :allow))
        // Effect is first, not last — should still be extracted
    ...
    fn test_and_middle_effect_to_when() {
        // (and pred1 (effect :allow) pred2) → (when (and pred1 pred2) (effect :allow))
    ...
    fn test_no_flatten_mixed_combinators() {
        // (and (or a b)) should NOT flatten — different combinators
