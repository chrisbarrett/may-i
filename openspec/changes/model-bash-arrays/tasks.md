...
- [ ] 1.1 Add a failing parser test: `arr=(a b c); echo done` parses both the
...
- [ ] 4.2 Fuzz/proptest: the parser and evaluator do not panic on arbitrary
      array-ish input (unterminated `(`, nested `(`, malformed subscripts).
- [ ] 4.3 `cargo tarpaulin`; inspect `lcov.info` for uncovered branches in the
      array lexer/parser; add unit tests a proptest cannot reach.
- [ ] 4.4 Confirm no DSL/config/trust-hash surface changed; no migration. Verify
      this unblocks `resolve-constant-array-arguments` (array AST + separated
      subscript are available).
- [ ] 4.5 Review `REFERENCE.md`: document array-literal and subscript parsing /
