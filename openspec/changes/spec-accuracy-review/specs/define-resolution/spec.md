...
The define resolver SHALL collect named predicate references from all parts of an `Effect::If`: the predicate, the then-effect, and the else-effect. (CHANGED: define names are bare symbols, not keywords)
...
The define resolver SHALL collect named predicate references from all Cond branches and the optional fallback. (CHANGED: define names are bare symbols, not keywords)
...
The define resolver SHALL recurse into `Effect::And`, `Effect::Or`, and `Effect::Not` to collect named predicate references. (CHANGED: define names are bare symbols, not keywords)
...
`resolve_effect_predicates` SHALL recursively replace named predicates with their definitions through If, Cond, And, Or, and Not effect structures. (CHANGED: define names are bare symbols, not keywords)
