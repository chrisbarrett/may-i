...
- [ ] 1.1 Write `scripts/validate-change-doc-sync.sh` with `--self-test`
...
- [ ] 1.2 Implement the trigger algorithm from design.md D4: for each
...
- [ ] 1.3 `chmod +x` the script and confirm `scripts/validate-change-doc-sync.sh --self-test` is green.

## 2. Wire into prek

- [ ] 2.1 Add a `validate-change-doc-sync` hook to `prek.toml` after
...
- [ ] 3.1 Apply the `spec-conventions` delta: add the new requirement to
      `openspec/specs/spec-conventions/spec.md` under `## Requirements`.
- [ ] 3.2 Add a `rules.tasks:` reminder bullet to `openspec/config.yaml`:
      a change with a user-facing spec delta must carry a REFERENCE.md
      consideration task.

## 4. Documentation impact (REFERENCE.md consideration)

- [ ] 4.1 Review REFERENCE.md for impact from this change. Resolution:
...
- [ ] 5.1 In `openspec/changes/quantifier-sequence-groups/tasks.md`, add a
...
- [ ] 6.1 `scripts/validate-change-doc-sync.sh --self-test` green.
- [ ] 6.2 Run the validator over live changes; confirm it now flags any
      user-facing change lacking a REFERENCE.md task and passes this
      change and the amended `quantifier-sequence-groups`.
- [ ] 6.3 `scripts/validate-spec-frontmatter.sh` still green (spec-conventions edit).
- [ ] 6.4 `openspec validate doc-sync-reference-gate --strict` passes.
- [ ] 6.5 `prek run --all-files` (or the pre-commit subset) green.
