...
- [x] 1.1 Copy *All CLI subcommands have integration tests* — body and
  `#### Scenario:` children — verbatim from
  `integration-test-coverage/spec.md` into `testing-strategy/spec.md`.
- [x] 1.2 `rm -rf openspec/specs/integration-test-coverage/`.

## 2. Fold `migration-testing` into `migration-system`

- [x] 2.1 Append a brief sentence to the `migration-system` Purpose
...
- [x] 2.2 Copy each of the 6 requirements verbatim from
...
- [x] 2.3 `rm -rf openspec/specs/migration-testing/`.

## 3. Update cross-references

- [x] 3.1 `grep -rln 'integration-test-coverage\|migration-testing' openspec/specs/ .claude/ CONTEXT.md CLAUDE.md README.md`
  — repoint each hit.

## 4. Verify

- [x] 4.1 Each absorbed requirement appears once and only once in its
  new home.
- [x] 4.2 `testing-strategy` retains its `Contributor-only.` Purpose
  ...
  test setup).
- [x] 4.3 `openspec validate consolidate-testing-specs --strict` passes.
