...
#### Scenario: Mixed file preserves v2 forms

- **GIVEN** a config file with a mix of v1 `(has …)` and v2 `(fact? …)` forms
- **WHEN** migration runs
- **THEN** only v1 forms are rewritten; existing v2 forms are byte-identical after migration.
...
#### Scenario: Random compound config round-trips

- **GIVEN** a randomly generated v1 compound config
- **WHEN** migration produces a v2 config
- **THEN** the v1 and v2 configs SHALL produce identical evaluation results for every randomly generated input.
