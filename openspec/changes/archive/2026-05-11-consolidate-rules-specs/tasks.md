...
- [x] 1a.1 *Check evaluation propagates errors* — copy verbatim. The
...
- [x] 1b.1 *EvalResult exposes per-segment decisions* — copy verbatim.
- [x] 1b.2 *Segment decisions describe non-overlapping byte ranges* —
  copy verbatim.
- [x] 1b.3 *Display does not re-evaluate to colourise* — copy verbatim.
- [x] 1b.4 *Aggregate decision unchanged* — copy verbatim.

## 2. Update `rule-evaluation` Purpose

- [x] 2.1 Append a sentence to the existing Purpose noting that
...
- [x] 3.1 `rm -rf openspec/specs/evaluator-error-handling/`
- [x] 3.2 `rm -rf openspec/specs/eval-segment-decisions/`

## 4. Update cross-references

- [x] 4.1 `grep -rln 'evaluator-error-handling\|eval-segment-decisions' openspec/specs/ .claude/ CONTEXT.md CLAUDE.md README.md`
...
- [x] 5.1 Each absorbed requirement appears once and only once in
  `rule-evaluation`.
- [x] 5.2 `openspec validate consolidate-rules-specs --strict` passes.

## 6. Note: rename deferred

- [x] 6.1 The rename `rule-evaluation` → `rule-decisions` (vocab
