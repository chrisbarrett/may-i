...
- [ ] 1.1 Integration test: bypass reproducer.
...
- [ ] 1.2 Integration test: `:via`-gated deny survives wrapper chain.
      `(rule "sudo" (authorise #cmd))` + `(rule "bash" (authorise #cmd))` +
...
- [ ] 2.1 Confirm `Effect::Authorise`'s call site can observe
      ...
      `pub` and `Clone`.
- [ ] 2.2 Audit other `as_joined()` consumers (`(matches? #var …)`,
...
- [ ] 3.1 Add `evaluate_authorised_tokens(tokens: &[String], config,
...
- [ ] 3.2 Body:
      ...
      - else: push `:via` into facts; call
        ...
        no parse.
- [ ] 3.3 Decide which characters count as "shell metacharacter" for
...
- [ ] 4.1 Move the empty-binding short-circuit out of the
...
- [ ] 4.2 In `recurse_into_bound_command`, match on the
      `BindingValue`:
...
- [ ] 4.3 `evaluate_tail_authorise_fold` (`ArgPattern::Tail`): the
...
- [ ] 4.4 `recurse_into_inner_command` (`ParameterForm::Authorise`)
      and `entry.rs`'s `ParameterTreatment::Authorise` pre-rule
      recursion: NO change. Captured value is always a single string.
- [ ] 4.5 Delete any newly-orphaned helpers or imports.

## 5. Property tests

- [ ] 5.1 Equivalence proptest in `command.rs`: for any token list
...
- [ ] 5.2 Bypass proptest: for any well-formed inner command `c` such
...
- [ ] 6.1 Apply the `parser-bindings` delta (string-vs-token-list split
      and new scenarios) to the live spec.
- [ ] 6.2 Apply the `parameter-many-till` delta (clarifying sentence
      about the asymmetry) to the live spec.
- [ ] 6.3 Re-read the scenario at the existing
...
- [ ] 7.1 `cargo fmt`, `cargo clippy`, `cargo test --workspace`.
- [ ] 7.2 User oracle: `may-i eval 'sudo bash -c "echo a && rm /tmp/x"'`
...
- [ ] 8.2 Bump version in `Cargo.toml` per the project's release
