...
- [x] 1.4 Write smart constructors on the producer side
...
- [x] 2.1 Move long-`(or …)` truncation from the renderer
...
- [x] 2.2 Move cond-branch collapse (the "5 branches, 2nd
...
- [x] 2.3 Move dimming of unevaluated branches from the
  renderer-side `dimmed` flag on `Doc` to a producer-side
  `Role::Dimmed` (or a `dimmed()` accessor on `TraceNode`).
- [x] 2.4 Move fact-query evidence compaction (presence vs
...
- [x] 2.5 Move the `ArgPattern → Doc` exhaustiveness arm
...
- [x] 3.1 Add a `TraceNode → Layout` pass in `src/output/`
  (replacing `transform.rs`'s `Doc<Option<Ann>>` → `Layout`
  walk). Walk consumes `TraceNode` via accessors only.
- [x] 3.2 Port `render_rule.rs` to attach right-column
  ...
  path.
- [x] 3.3 Port `mod.rs` re-exports and any direct `Ann::`
  matches in shared helpers.
- [x] 3.4 Verify the text-trace snapshots under `tests/`
  ...
  accepting.
- [x] 3.5 Confirm `rg 'Ann::' src/output/` returns zero
  hits.
- [x] 3.6 Confirm `rg 'may_i_core::pattern::(ArgPattern|CommandPattern|MatchMode|Quantifier)' src/output/`
  returns zero hits.

## 4. Port the JSON renderer

- [x] 4.1 Port `src/output/json.rs` to consume `TraceNode`.
  JSON renderer maps `Role` to field names and `Evidence`
  to nested objects.
- [x] 4.2 Preserve the user-observable JSON invariants
...
- [x] 4.3 Re-baseline JSON snapshots in `tests/snapshots/`
...
- [x] 7.7 `rg '<unknown-arg-pattern>' src/` returns zero
