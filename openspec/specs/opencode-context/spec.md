# opencode-context Specification

## Purpose

How `may-i eval` ingests OpenCode runtime facts: through explicit `--fact :client/opencode --fact :opencode/agent=X` flags only. Ambient environment variables (`MAYI_OPENCODE_AGENT`, `OPENCODE=1`) SHALL NOT synthesize facts. Rules gate on these via `(fact? …)` predicates and traces surface fact-driven decisions in both JSON and human-readable output.
