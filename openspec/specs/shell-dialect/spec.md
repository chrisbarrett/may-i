...
The shell dialect selects which grammar an input command is judged against, so
that a command written for the executing shell is parsed the way that shell
would run it. The dialect governs only which constructs are well-formed; it is
strictness-preserving — every zsh production can only remove a spurious
diagnostic or make a word parse, never widen a decision — and it does not
change which rules participate or how rules are hashed, so it is not
trust-relevant. It is observed ground truth (derived from the executing
shell), never a Fact: no `:dialect`/`:zsh` fact key exists and rules cannot
branch on it. The `shell-command-security-model` spec owns the
parser/diagnostic contract this capability feeds.
