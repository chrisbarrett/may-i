# wrapper-tail Specification

## Purpose

TBD — established by the dsl-coherence change. This capability defines wrapper-tail slicing: the parser-side `(tail (after …))` declaration that splits argv into outer/tail slices, and the rule-side `(tail (authorise))` form that recurses on the tail.
