...
`for` loop, the subject and pattern words of a `case`, or the operand of a
parameter-expansion operator (the `value`/`pattern`/`replacement`/`message`
text of forms such as `${x:-…}`, `${x:+…}`, `${x:=…}`, `${x:?…}`, `${x#…}`,
`${x%…}`, `${x/…/…}`). An embedded command SHALL NOT resolve to `:allow` merely
because the word that contains it is never reached by decomposition.
...
- **THEN** every command, backtick, and process substitution present in the
  input — including those inside parameter-expansion operands — SHALL be
  represented by an embedded-command evaluation unit
