...
---
# display-safe-output Specification

## Purpose

Contributor-only. Defines the type-enforced boundary that prevents
user-controlled control characters (notably `\x1b` ANSI sequences) in command
names, argv, regex actuals, captured values, and `(load)`ed config from reaching
a terminal. Escaping happens at a single choke point: `SafeText` (escape every
control character) and `SafeSource` (sanitise offset-addressed source while
preserving byte length) live in `may-i-core`; styling is carried as data (a
closed `Style` role enum) with the renderer's role→SGR table as the only ANSI
source; and a single sink owns every process output stream. See
`output-rendering` for the rendering surface and `code-quality` for the lint and
prek enforcement of the no-`colored` / single-sink invariants.
