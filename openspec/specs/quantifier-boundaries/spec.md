...
#### Scenario: Non-matching arg in sequence
- **WHEN** matching positional pattern `"file"+` against args `["file" "notfile"]`
- **THEN** it SHALL return matched=false (all remaining must match)
...
#### Scenario: Non-matching arg in sequence
- **WHEN** matching positional pattern `"x"*` against args `["x" "y"]`
- **THEN** it SHALL return matched=false (all remaining must match the pattern)
