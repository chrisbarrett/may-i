...
---
# rule-combination Specification

## Purpose

Defines how the engine combines effects from multiple matching rules
into a single decision. Replaces the older first-match-wins semantics
with a most-strict-wins lattice (`Allow < Ask < Deny`) so that
loaded or repo-local rules cannot widen the policy established by an
existing matching primary rule.
