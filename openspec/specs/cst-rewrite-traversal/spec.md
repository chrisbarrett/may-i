...
---
# cst-rewrite-traversal Specification

## Purpose

Contributor-only. Defines the CST rewrite-traversal seam in the `sexpr` crate:
a bottom-up (post-order) traversal combinator over the `ShapeF` / `CstNode`
functor that owns full-tree recursion, convergence iteration, and trivia/`Span`
preservation on behalf of migration passes. Individual passes match and rewrite
only the node they are given; recursion, fixed-point iteration, and comment/
whitespace threading live in the seam, built on production (non-test-gated)
functor and catamorphism primitives.
