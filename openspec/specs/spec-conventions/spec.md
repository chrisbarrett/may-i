...
Contributor-only. Document the conventions for authoring specs under `openspec/specs/`: heading structure, bucket assignment, audience separation, trust-relevance declaration, granularity threshold, the TBD ban, and the pre-merge checklist every spec-touching change applies. The first four buckets correspond to the four-layer model in `CONTEXT.md` (Rules, Facts, Parsing, Trust).
