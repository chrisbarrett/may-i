...
| Authorise | Recurse on an inner command. Spelled `(authorise)` — bare, no arguments. Appears nested in a host context: `(parameter NAME (authorise))`, `(tail (authorise))`, or as a positional element. Pushes a `:via` fact for the wrapping program.                               | `(parameter "c" (authorise))`, `(tail (authorise))`                |
| Tail      | The slice of argv after a wrapper's outer flags. Declared on the parser side as `(tail (after :flags))` or `(tail (after "--"))`; addressed on the rule side as `(tail (authorise))`. When a parser declares a tail, argv matchers in the rule body scope to outer only.  | `(parser "sudo" (style gnu) (tail (after :flags)))`                |
...
| Parser    | A per-program declaration that picks a Style, lists which flags carry values, and optionally declares a tail. Programs without one default to `gnu`. Common wrappers (sudo, xargs, env, …) ship pre-declared in the prelude.                                              | `(parser "kubectl" (style gnu) (parameter ["n" "namespace"]))`     |
