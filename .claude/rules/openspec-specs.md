...
7. Stable spec carries YAML frontmatter (`audience: user|contributor`, `bucket: <kebab>`, optional `trust-relevant: true`). Validator: `scripts/validate-spec-frontmatter.sh` (runs via prek). Discovery (per-file loop; mikefarah yq can't merge multi-file frontmatter cleanly): `for f in openspec/specs/*/spec.md; do [ "$(yq --front-matter=extract .audience "$f")" = user ] && echo "$f"; done`.
